...
//		private int waitForMilliSeconds = 20;
		...
		// private float x_offset_left = 100.0f;



		private float x_offset_left = 0;
        private float y_offset_up = 0;
				    	...
				    	for(float idx_pos_y = initial_pos_y; idx_pos_y < final_pos_y; idx_pos_y += update_pos_y)
				    	{
				    		for(float idx_pos_x = initial_pos_x; idx_pos_x < final_pos_x; idx_pos_x += update_pos_x)
				    		{
						    	for(float idx_pos_z = initial_pos_z; idx_pos_z < final_pos_z; idx_pos_z += update_pos_z)
						    	{
									...
									// old Code for saving image as png,

									// Vector2 vec = new Vector2(100,100);
									// Debug.Log("vec "+ vec + "Size X/y plane " + (int)sizeX_plane + " " +  (int)sizeY_plane);
									...
									// //ScreenCapture.CaptureScreenshot(Path.Combine(imagePath, "planeeee" + screenshotIndex + ".png"), 2);

									// Debug.Log("screenshotIndex2 "+ screenshotIndex);
									//screenshotIndex++;

									if (p_communication.rpc != null)
									{
										// Call the function from ClassA
										// Debug.Log(p_communication.rpc.x);
										p_communication.rpc.speak();
										Debug.Log("ab");
									}
									else
									{
										Debug.LogError("ClassA instance is not available.");
									}
